...
public class PlayerAnimationController : MonoBehaviourPun
{
    ...
    public void UpdateAnimatorStates()
    {
        ...
        HandlePipeAnimation(controller.state);
    ...
    void HandlePipeAnimation(Enums.PowerupState state)
    {
        if (!photonView.IsMine)
            return;
        ...
        if (state == Enums.PowerupState.MiniMushroom && pe.miniOnly)
        {
            HandlePipeAnimation(pe);
        }
        else if(!pe.miniOnly)
        {
            HandlePipeAnimation(pe);
    ...
    void HandlePipeAnimation(PipeManager pe){
